...
| `--dkim-selector`, `--dkim-domain` | Compose a DKIM lookup from selector + base domain, fetch the `_domainkey` TXT record, extract the DKIM public key, and report the DNSSEC status. |
...
Extract a DKIM public key while validating the TXT record with DNSSEC:

```bash
dotnet run -- --dkim-domain whitehouse.gov --dkim-selector selector1 --server https://cloudflare-dns.com/dns-query
```
...
- When `--dkim-selector` and `--dkim-domain` are supplied, the tool queries `<selector>._domainkey.<domain>` as TXT, extracts the DKIM `p=` value, and reports it alongside the DNSSEC validation outcome.
...
- DKIM lookups require TXT records. Pair them with `--require-signed` if you need the TXT response to be DNSSEC signed; otherwise the tool will report `Unsigned` for zones that do not publish signatures.
